...
describe("draft submission in flight", () => {
  beforeEach(() => {
    useInputStore.getState().setDraftSubmissionInFlight(null)
  })

  test("starts empty so a fresh draft screen shows its own welcome", () => {
    expect(useInputStore.getState().draftSubmissionInFlight).toBeNull()
  })

  test("holds the submitted text verbatim, including leading whitespace and newlines", () => {
    const submitted = "  primeira linha\n\nsegunda linha  "
    useInputStore.getState().setDraftSubmissionInFlight(submitted)
    expect(useInputStore.getState().draftSubmissionInFlight).toBe(submitted)
  })

  test("clears on the send-failure path so the draft screen cannot pulse forever", () => {
    useInputStore.getState().setDraftSubmissionInFlight("prompt que falhou")
    expect(useInputStore.getState().draftSubmissionInFlight).not.toBeNull()

    // Mirrors the ChatInput catch handler, which restores the composer text and
    // must take the pending bubble down with it.
    useInputStore.getState().setDraftSubmissionInFlight(null)
    expect(useInputStore.getState().draftSubmissionInFlight).toBeNull()
  })

  test("does not disturb the other pending composer slots", () => {
    useInputStore.getState().setPendingInputText("rascunho normal")
    useInputStore.getState().setDraftSubmissionInFlight("prompt enviado")

    expect(useInputStore.getState().pendingInputText).toBe("rascunho normal")
    expect(useInputStore.getState().pendingPresetSubmit).toBeNull()

    useInputStore.getState().setDraftSubmissionInFlight(null)
    expect(useInputStore.getState().consumePendingInputText()).toEqual({
      text: "rascunho normal",
      mode: "replace",
    })
  })
})
