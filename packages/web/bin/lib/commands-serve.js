...
import { buildLocalUrl, resolveServeHost, assertSafeBrowserPort, resolveServeUiPassword, resolveServeUiPasswordHash, assertAuthenticatedNetworkExposure } from './cli-network.js';
...
async function serveCommand(options) {
    ...
    const uiPasswordHash = resolveServeUiPasswordHash(options);
    const resolvedUiPassword = uiPasswordHash
      ? { password: undefined, generated: false }
      : resolveServeUiPassword(options);
    ...
    if (!effectiveUiPassword && !uiPasswordHash && !options.suppressUiPasswordWarning) {
    ...
    if (options.foreground) {
      ...
      writeInstanceOptions(fgInstanceFilePath, {
        ...
        uiPassword: effectiveUiPassword,
        uiPasswordHashConfigured: Boolean(uiPasswordHash),
        apiOnly: options.apiOnly === true,
      }, emitNotice);
    ...
    // Exactly one UI credential reaches the server: the hash (never alongside
    // the plaintext) or the resolved plaintext password.
    const inheritedEnv = { ...process.env };
    delete inheritedEnv.OPENCHAMBER_UI_PASSWORD;
    delete inheritedEnv.OPENCHAMBER_UI_PASSWORD_HASH;
    const child = spawn(runtimeBin, serverArgs, {
      ...
      env: {
        ...
        ...(uiPasswordHash ? { OPENCHAMBER_UI_PASSWORD_HASH: uiPasswordHash } : {}),
